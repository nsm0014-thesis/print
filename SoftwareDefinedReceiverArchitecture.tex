...
These error states are then mapped to measurement residuals using the observation matrix, \(\mathbf{H}\) (Equation~\ref{eq:Hmat},~\ref{eq:Y}).\(\mathbf{H}\) is defined by the Jacobian of \(\mathbf{Y}\) with respect to \(\delta\hat{\mathbf{x}}\), or in a mathematical form,
...
The last piece in WNLS algorithm is defining the weights, \(\mathbf{W}\).\(\mathbf{W}\) is utilized by the algorithm to place more trust in satellites with high \(C/N_0\) compared to satellites with lower \(C/N_0\). The weighting matrix is shown in Equation~\ref{eq:weights}.
...
is the variance of the pseudorange-rate measurement from the \(j^{th}\) satellite. In Equations~\ref{eq:psrVar} and~\ref{eq:psrdotvar}, \(\lambda_{code}\) is the wavelength of data signal and \(\lambda_{carrier}\) is the wavelength of the carrier signal. In this case, \(C/N_0\) is specified in Hertz.
...
Once an initial receiver position has been found, vector tracking channels can open and closed form PVT solution can start to be processed. Vector tracking loops and algorithms are discussed in the next chapter.
