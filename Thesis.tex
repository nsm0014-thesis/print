...
% Why use Strip Theory?

% How does Strip Theory Work?
As mentioned earlier, the surface area of the aircraft can be difficult to estimate because of the complex shapes that form the aircraft in its final form. In isolation, the wings and other lifting surfaces (\i.e.\ tail or canard) can evaluated using a Computer-aided design or CFD program. However, when meshed with the other aircraft components like the fuselage and attached propellers, lift and drag forces can be to complicated to calculate without a numerical solver. Strip Theory discretizes the aircraft into thousands (millions, depending on the size of the aircraft) or rectangles with known areas. Each of these discretized squares are run through a CFD simulation at various angles of attack, sideslip and velocities and the results are saved to a database. This database is then queried during simulation based on each rectangle's orientation at the current time step.
